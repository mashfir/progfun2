 ...
 */
 class ListsSuite {
   ...
   * when writing tests.
   */
  @Test def `details why one plus two is three (0pts)`: Unit = {
  ...
  @Test def `max of large negative numbers (10pts)`: Unit = {
    Assert.assertEquals(max(List(-10, 7, 0)), 7)
  }

  @Test def `max of all negative numbers (10pts)`: Unit = {
    Assert.assertEquals(max(List(-10, -2, -1)), -1)
  }

  @Test def `max throws an exception if its argument is empty list`: Unit = {
    try {
      max(List())
      Assert.fail("No exception has been thrown")
    } catch {
